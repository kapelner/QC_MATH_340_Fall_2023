...
\inred{
Our strategy is to use Cochran's theorem. Let $\bv{Z} = \bracks{Z_1~Z_2}^\top$. We now want a quadratic form with vector $\Z$, so we need to guess a determining matrix which we call $B_1$ and show this is an equivalent expression for our target rv $X$:
