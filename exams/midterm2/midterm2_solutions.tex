...
\begin{align*}
p_Y(y) = p_X(g^{-1}(y)) = p_X(e^y - 1) =& \binom{n}{e^y - 1} p^{e^y - 1} (1-p)^{n - e^y + 1} \indic{e^y - 1 \in \{0, 1, \ldots, n\}} \\
=& \binom{n}{e^y - 1} p^{e^y - 1} (1 - p)^{n - e^y + 1} \indic{y \in \{\ln{(1)},\, \ln{(2)},\, \ldots\,,\, \ln{(n + 1)}\}}
\end{align*}
}
...
\beqn
X = \half U^2 = \parens{\frac{1}{\sqrt{2}} U}^2.
\eeqn

We know based on our transformation equations for the normal that $\frac{1}{\sqrt{2}} U \sim \stdnormnot$ which means $X \sim \chisq{1}$.


\fbox{Via Cochran's theorem} Let $\bv{Z} = \bracks{Z_1~Z_2}^\top$. We now want a quadratic form with vector $\Z$, so we need to conjure a determining matrix which we call $B_1$ and show this is an equivalent expression for our target rv $X$:

\vspace{-0.3cm}
\beqn
X = \bv{Z}^\top B_1 \bv{Z} &=& \bracks{Z_1~Z_2} \underbrace{\twobytwomat{a}{b}{c}{d}}_{B_1} \twovec{Z_1}{Z_2} =  \bracks{Z_1~Z_2} \twovec{a Z_1 + b Z_2}{c Z_1 + d Z_2} \\
&=& aZ_1^2+bZ_1Z_2+cZ_1Z_2+dZ_2^2\\
&=& aZ_1^2+(b+c)Z_1Z_2+dZ_2^2
\eeqn

To match our target rv $X$: $a=\frac{1}{2}$, $(b+c)=1$, and $d=\frac{1}{2}$. We add the condition that $b=c$ as $B_1$ must be symmetric, implying $b=c=1/2$. Putting it together,
$$B_1=\twobytwomat{1/2}{1/2}{1/2}{1/2}$$
This matrix $B_1$ has $\rank{B_1} = 1$. To use Cochran's theorem, we conjure a matrix $B_2$ such that $B_1+B_2=I_2$:
