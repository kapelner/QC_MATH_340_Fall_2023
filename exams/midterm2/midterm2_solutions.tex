...
The following is the density of the normal: $\mathcal{E} \sim \stdnormnot = \oneoversqrt{2\pi}e^{-\epsilon^2 / 2}$. Error distributions are defined by three conditions
...
\beqn
\frac{\partial h_1}{\partial M} = U, ~~\frac{\partial h_1}{\partial U} = -M / U^2, ~~\frac{\partial h_2}{\partial M} = 0, ~~\frac{\partial h_2}{\partial U} = 1, ~~ |J_h| = |(U)(1) - (-M/U^2)(0)| = |U|
\eeqn 

Putting it all together, $f_M(m) = \int_\reals f_{X_1, X_2}(h_1, h_2)|J_h| du = \int_\reals f_{X_1, X_2}\parens{\frac{m}{u}, u} |u| \,du$.


\fbox{Answer \#2} Let $U = X_1$ thus $X_1 = h_1(M, U) = U$. Then we know $M = U X_2$ thus $X_2 = h_2(M, U) = M / U$. We then have 

\beqn
\frac{\partial h_1}{\partial M} = 0, ~~\frac{\partial h_1}{\partial U} = 1, ~~\frac{\partial h_2}{\partial M} = U, ~~\frac{\partial h_2}{\partial U} = -M / U^2, ~~ |J_h| = |(0)(-M / U^2) - (1)(U)| = |U|
\eeqn 
...
where the $\convd \stdnormnot$ above follows from Slutsky's theorem $\textcircled{\tiny{A}}$.
...
\inred{
The $X_n \sim \text{Weibull}(k, n)$ rv is non-negative with CDF $F_{X_n}(x) =  1 - e^{-(nx)^k}$ and thus $S_{X_n}(x) =  e^{-(nx)^k}$. By the definition, $X_n \convp 0$ means:
...
\beqn
\limitn \prob{|X_n - 0| > \epsilon} = \limitn \prob{X_n > \epsilon} = \limitn S_{X_n}(\epsilon) = \limitn e^{-(n\epsilon)^k} = 0
\eeqn

where the first equality above follows from the non-negativity of $X$.
...
Since $3.14 > 2\mu = 2\cdot0.294 = 0.588$ we can employ this inequality and substitute to find:
...
\fbox{Acceptable Answer} By Markov's inequality and the fact that $X_1, X_2$ are both non-negative so their convolution is non-negative, we have
...
\subquestionwithpoints{10} Let $Z_1, Z_2 \iid \stdnormnot$. Let $X = \half Z_1^2 + Z_1 Z_2 + \half Z_2^2$. How is $X$ distributed? Hint: use Cochran's thm.\\

\inred{
Let $\bv{Z} = \bracks{Z_1~Z_2}^\top$. By playing around, we conjure a $B_1$ matrix so that $X$ is expressed as a quadratic form in the standard normal vector $\Z$:

\beqn
X = \bv{Z}^\top B_1 \bv{Z} &=& \bracks{Z_1~Z_2} \underbrace{\twobytwomat{1/2}{1/2}{1/2}{1/2}}_{B_1} \twovec{Z_1}{Z_2}\\
&=& \bracks{Z_1~Z_2} \twovec{\half Z_1 + \half Z_2}{\half Z_1 + \half Z_2} \\
&=& \half Z_1^2 + \half Z_1 Z_2 + \half Z_2 Z_1+ \half Z_2^2 \\
&=& \half Z_1^2 + 2 Z_1 Z_2 + \half Z_2^2
\eeqn
...
which has $\rank{B_2} = 1$ which can be seen by taking the negative of column 2 and noticing it is the same as column 1. Since $B_1 + B_2 = I_2$ and  $\rank{B_1} + \rank{B_2} = 2$, we can use Cochran's Thm's result that the quadratic forms are chi-squared distributed and thus:
