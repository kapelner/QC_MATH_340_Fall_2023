...
\subquestionwithpoints{13} Let $Y\,|\,X = x \sim \gammanot{x + 1}{\beta}$ and $X \sim \geometric{p}$. Find $f_Y(y)$ and identify it as one of the brand name rv's we studied and identify its parameter(s). Hint: $e^{a} = \displaystyle \sum_{i=1}^\infty \frac{a^i}{i!}$. Advice: leave this problem for last. 
...
\inred{
Any subset of a multivariate normal is itself multivariate normal. A subset of dimension one is thus normal:

\beqn
X_2 &\sim& \normnot{\mu_2 = 1}{\bSigma_{2,2} = 2} \\
f_{X_2}(x) &=& \oneoversqrt{4\pi} e^{-\oneover{4} (x - 1)^2}
\eeqn
}
\pagebreak
