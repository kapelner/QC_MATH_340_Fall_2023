...
\hardsubproblem{If $X \sim \gammanot{\alpha}{\beta}$ and $Y\,|\,X = x \sim \exponential{x}$, show that $Y \sim \text{Lomax}(\beta, \alpha)$. See the Lomax's Wikipedia article  by clicking \href{https://en.wikipedia.org/wiki/Lomax_distribution}{here}. Kernels always make it easier.}\spc{4}
