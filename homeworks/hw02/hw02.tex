...
\easysubproblem{[MA] Let $p_x = p_y$. Find $\prob{X > Y}$ in a different way by finding $\prob{X = Y}$ and then using the law of total probability.}\spc{10}
...
\intermediatesubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{0, c}}\mathrm{d}x$ where $c,d \in \reals$ and $c \geq 1$.}\spc{1}

\intermediatesubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{c, c + 1}}\mathrm{d}x$ where $c,d \in \reals$. and $c > 0$.}\spc{1}
...
\easysubproblem{Find the expectation of $T \sim \erlang{k}{\lambda}$. Use the expectation of the exponential rv and the linearity rules!}\spc{1}

\easysubproblem{Find the variance of $T \sim \erlang{k}{\lambda}$. Use the variance of the exponential rv and and the linearity rules!}\spc{3}
...
\hardsubproblem{For continuous rv $X \sim f_X(x)$, given the Fourier inversion theorem, if $\phi_X(t) \in L^1$ then prove P6. Hint: translate the Fourier formulas (the ones with $g$ and $\hat{g}$ into our vocabulary and then use $u$-substitions.

SOLUTION. We begin with the statement of the forward Fourier transform. If $g \in L^1$,
...
We translate this to our vocabulary in the following way, let $f \rightarrow g, x \rightarrow t, v \rightarrow \omega, \alpha \rightarrow \hat{g}, $. The two statements then become:

\beqn
\alpha(v)   &=& \int_\reals e^{-i2\pi v x} f(x) dx \\
f(x)           &=& \int_\reals e^{i2\pi v x} \alpha(v) dv.
\eeqn
...
\beqn
\alpha(-t / (2\pi))   &=& \int_\reals e^{i t x} f(x) dx \\
f(x)                       &=& \int_\infty^{-\infty} e^{-i t x} \alpha(-t / (2\pi)) \parens{-\oneover{2\pi}} dt \\
&=& \oneover{2\pi} \int_\reals e^{-i t x} \alpha(-t / (2\pi)) dt.
\eeqn

Letting $\phi_X(t) := \alpha(-t / (2\pi))$ completes the proof:
